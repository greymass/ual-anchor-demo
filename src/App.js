...
class TestApp extends Component {
  ...
  renderLoggedInView = () => (
    <>
      ...
      {(this.props.ual.activeUser)
        ? (
          <p>{this.props.ual.activeUser.accountName}@{this.props.ual.activeUser.requestPermission}</p>
        )
        : false
      }

      <button type='button' onClick={this.transfer} style={{ ...styles.button, ...styles.blueBG }}>
        <p style={styles.baseText}>{`Transfer 0.0001 EOS`}</p>
      </button>
  ...
  renderLoginButton = () => (
    <button type='button' onClick={this.props.ual.showModal} style={styles.button}>
      <p style={{buttonText: styles.buttonText, baseText: styles.baseText}}>LOGIN w/ Jungle Account</p>
    </button>
  )
  ...
  render() {
    ...
    return (
      <div style={styles.container}>
        {activeUser ? this.renderLoggedInView() : this.renderLoginButton()}
      </div>
    )
  }
}
...
const anchor = new Anchor([exampleNet], {
  appName: 'ual-anchor-demogi'
})

const App = () => (
  <UALProvider chains={[exampleNet]} authenticators={[anchor]} appName='Anchor + Authenticator Test App'>
    <TestAppConsumer />
  </UALProvider>
)
